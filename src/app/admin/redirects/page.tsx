    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">{t(`${redirects.length} regole configurate`, `${redirects.length} rules configured`)}</p>
        </div>
      ...
      <Card>
        <div className="divide-y">
          ...
          {redirects.length === 0 ? (
            <div className="p-12 text-center text-muted-foreground">{t('Nessun redirect configurato.', 'No redirects configured.')}</div>
          ) : (
            redirects.map((r) => (
              <div key={r.id} className="grid grid-cols-[2fr_2fr_auto_auto_auto_auto] gap-4 items-center px-6 py-4 hover:bg-muted/40">
                ...
                <Badge variant={r.enabled ? 'success' : 'outline'}>{r.enabled ? t('Attivo', 'Active') : t('Disattivo', 'Disabled')}</Badge>
